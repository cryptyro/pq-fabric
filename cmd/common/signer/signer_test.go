...
func TestXDSASigner(t *testing.T) {
	conf := Config{
		MSPID:        "SampleOrg",
		IdentityPath: filepath.Join("testdata", "signer", "xdsa.pem"),
		KeyPath:      filepath.Join("testdata", "signer", "xdsa_sk"),
	}
	...
	require.NoError(t, err)
	verify := xdsa.Verify(signer.key.(*xdsa.PrivateKey).Public().(*xdsa.PublicKey), msg, sig)
	require.True(t, verify)
}
