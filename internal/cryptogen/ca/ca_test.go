...
func TestLoadCertificateEDMLDSA(t *testing.T) {
	testDir := t.TempDir()
	...
	priv, err := csp.GeneratePrivateKey(certDir, EDMLDSA)
...
func TestLoadCertificateEDMLDSA_wrongEncoding(t *testing.T) {
	testDir := t.TempDir()
...
func TestLoadCertificateEDMLDSA_empty_DER_cert(t *testing.T) {
	testDir := t.TempDir()
...
func TestGenerateSignCertificate(t *testing.T) {
	...
	privGeneric, err := csp.GeneratePrivateKey(certDir, EDMLDSA)
	require.NoError(t, err, "Failed to generate signed certificate")
	priv := privGeneric.(*xdsa.PrivateKey)
	...
	cert, err = rootCA.SignCertificate(certDir, testName, ous, nil, priv.Public(),
	...
	cert, err = rootCA.SignCertificate(certDir, testName, nil, sans, priv.Public(),
		x509.KeyUsageDigitalSignature, []x509.ExtKeyUsage{})
	require.NoError(t, err)
	...
	_, err = rootCA.SignCertificate(certDir, "empty/CA", nil, nil, priv.Public(),
	...
	_, err = badCA.SignCertificate(certDir, testName, nil, nil, &xdsa.PublicKey{},
		x509.KeyUsageKeyEncipherment, []x509.ExtKeyUsage{x509.ExtKeyUsageAny})
