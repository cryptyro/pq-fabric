...
func TestLoadPrivateKey(t *testing.T) {
	testDir := t.TempDir()
	priv, err := csp.GeneratePrivateKey(testDir, EDMLDSA)
	if err != nil {
...
func TestLoadPrivateKey_BadPEM(t *testing.T) {
	...
	for _, test := range []struct {
		...
		},
		{
			name:   "not EC key",
			data:   pkcs8RSAPem,
			errMsg: fmt.Sprintf("%s: pem bytes do not contain an ECDSA nor ed25519 nor xdsa private key", badPEMFile),
		},
		{
...
func TestXDSASigner(t *testing.T) {
	pub, priv, err := xdsa.GenerateKey(rand.Reader)
	if err != nil {
		t.Fatalf("Failed to generate private key: %s", err)
	}

	signer := csp.XDSASigner{
		PrivateKey: priv,
	}
	require.Equal(t, priv.Public(), signer.Public().(*xdsa.PublicKey))
	msg := []byte{1}
	sig, err := signer.Sign(rand.Reader, msg, nil)
	if err != nil {
		t.Fatalf("Failed to create signature: %s", err)
