...
func parsePrivateKeyPEM(rawKey []byte) (crypto.PrivateKey, error) {
	...
	_, isXdsa := key.(*xdsa.PrivateKey)
	if !isEcdsa && !isEd25519 && !isXdsa {
		return nil, errors.New("pem bytes do not contain an ECDSA nor ed25519 nor xdsa private key")
	}
...
type XDSASigner struct {
	PrivateKey *xdsa.PrivateKey
}

// Public returns the xdsa.PublicKey associated with PrivateKey.
func (e *XDSASigner) Public() crypto.PublicKey {
	return e.PrivateKey.Public()
}

// Sign signs the digest
func (e *XDSASigner) Sign(rand io.Reader, msg []byte, opts crypto.SignerOpts) ([]byte, error) {
	return (e.PrivateKey).Sign(rand, msg, opts)
}
