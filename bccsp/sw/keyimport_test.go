...
func TestXDSAPrivateKeyImportOptsKeyImporter(t *testing.T) {
	t.Parallel()

	ki := xdsaPrivateKeyImportOptsKeyImporter{}
	...
	_, err = ki.KeyImport([]byte{0}, &mocks2.KeyImportOpts{})
	require.Error(t, err)
	require.Contains(t, err.Error(), "Failed converting PKIX to XDSA public key")
	...
	_, err = ki.KeyImport(raw, &mocks2.KeyImportOpts{})
	require.Error(t, err)
	require.Contains(t, err.Error(), "Failed casting to XDSA private key. Invalid raw material.")
}
...
func TestXDSAGoPublicKeyImportOptsKeyImporter(t *testing.T) {
	t.Parallel()

	ki := xdsaGoPublicKeyImportOptsKeyImporter{}

	_, err := ki.KeyImport("Hello World", &mocks2.KeyImportOpts{})
	require.Error(t, err)
	require.Contains(t, err.Error(), "Invalid raw material. Expected *xdsa.PublicKey.")
...
func TestX509PublicKeyImportOptsKeyImporter(t *testing.T) {
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "Certificate's public key type not recognized. Supported keys: [ECDSA, ED25519, XDSA, RSA]")
}
