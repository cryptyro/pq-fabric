...
type xdsaKeyGenerator struct{}

func (kg *xdsaKeyGenerator) KeyGen(opts bccsp.KeyGenOpts) (bccsp.Key, error) {
	_, privKey, err := xdsa.GenerateKey(rand.Reader)
	if err != nil {
		return nil, fmt.Errorf("Failed generating XDSA key: [%s]", err)
	}

	return &xdsaPrivateKey{privKey}, nil
}
