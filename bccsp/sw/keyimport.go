...
type xdsaPrivateKeyImportOptsKeyImporter struct{}

func (*xdsaPrivateKeyImportOptsKeyImporter) KeyImport(raw interface{}, opts bccsp.KeyImportOpts) (bccsp.Key, error) {
	der, ok := raw.([]byte)
	if !ok {
		return nil, errors.New("[XDSADERPrivateKeyImportOpts] Invalid raw material. Expected byte array.")
	}

	if len(der) == 0 {
		return nil, errors.New("[XDSADERPrivateKeyImportOpts] Invalid raw. It must not be nil.")
	}

	lowLevelKey, err := derToPrivateKey(der)
	if err != nil {
		return nil, fmt.Errorf("Failed converting PKIX to XDSA public key [%s]", err)
	}

	xdsaSK, ok := lowLevelKey.(*xdsa.PrivateKey)
	if !ok {
		return nil, errors.New("Failed casting to XDSA private key. Invalid raw material.")
	}

	return &xdsaPrivateKey{xdsaSK}, nil
}
...
type xdsaGoPublicKeyImportOptsKeyImporter struct{}

func (*xdsaGoPublicKeyImportOptsKeyImporter) KeyImport(raw interface{}, opts bccsp.KeyImportOpts) (bccsp.Key, error) {
	lowLevelKey, ok := raw.(*xdsa.PublicKey)
	if !ok {
		return nil, errors.New("Invalid raw material. Expected *xdsa.PublicKey.")
	}

	return &xdsaPublicKey{lowLevelKey}, nil
}
...
func (ki *x509PublicKeyImportOptsKeyImporter) KeyImport(raw interface{}, opts bccsp.KeyImportOpts) (bccsp.Key, error) {
	...
	case *xdsa.PublicKey:
		return ki.bccsp.KeyImporters[reflect.TypeOf(&bccsp.XDSAGoPublicKeyImportOpts{})].KeyImport(
			pk,
			&bccsp.XDSAGoPublicKeyImportOpts{Temporary: opts.Ephemeral()})
	case *rsa.PublicKey:
	...
	default:
		return nil, errors.New("Certificate's public key type not recognized. Supported keys: [ECDSA, ED25519, XDSA, RSA]")
	}
}
